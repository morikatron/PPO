...
class PPO(tf.Module):
    ...
    def _build_model(self):
        ...
        # 枝分かれした先でfcレイヤーを追加する場合は下2行のコメントアウトを外し、三行目をコメントアウトします。
        #policy_dense = tf.keras.layers.Dense(units=self.config.num_unit, activation="relu")(h2)
        #policy = tf.keras.layers.Dense(self.num_actions, activation="softmax")(policy_dense)
        policy = tf.keras.layers.Dense(self.num_actions, activation="softmax")(h2)

        #value_dense = tf.keras.layers.Dense(units=64, activation="relu")(h2)
        #value = tf.keras.layers.Dense(1)(value_dense)
